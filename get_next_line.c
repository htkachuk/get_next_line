...
static int	ft_check(char **line, t_list *current, t_list **list)
{
	int		k;
	char	*temp;
	...
	*line = ft_strsub((char*)current->content, 0, k);
	temp = ft_strsub((char*)current->content, (k + 1),
...
static int	ft_r_ch(t_list **current, int fd, char **line, t_list **list)
{
	...
	while (ret == 3 && (k = read(fd, str, BUFF_SIZE)) > 0)
	{
		...
		free(temp);
		ret = ft_check(line, *current, list);
	}
	if (ret == 3 && (*line = (char*)(*current)->content))
		if ((ret = ft_check(line, *current, list)) == 3)
		{
			ret = 1;
			ft_memdel(&(*current)->content);
			(*current)->content = "\0";
